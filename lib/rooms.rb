module Rooms
  def self.commands
    [{
      /\A(look|ls)\Z/i => lambda { show_room }
  ...
  def show_room
    ...
    pc.room.items.each do |item|
      puts "There is a(n) ".red + item.name.blue + " here."
    end
    
    (pc.room.characters - [pc]).each do |char|
      puts char.name.blue + " is here.".red
    end
